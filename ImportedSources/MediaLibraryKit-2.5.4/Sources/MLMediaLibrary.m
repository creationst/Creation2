...
//        [self _setupLibraryPathPriorToMigration];
        self.libraryBasePath = [self documentFolderPath];
...
- (void)deleteAllDatabase
{
    ...
    NSURL *walURL = [[[self persistentStoreURL] URLByDeletingPathExtension] URLByAppendingPathExtension:@"sqlite-wal"];
    NSURL *shmURL = [[[self persistentStoreURL] URLByDeletingPathExtension] URLByAppendingPathExtension:@"sqlite-shm"];
    
    BOOL result = YES;
    NSFileManager *fm = [NSFileManager new];
    
    for (NSURL *url in @[[self persistentStoreURL], walURL, shmURL]) {
        if ([fm fileExistsAtPath:url.path]) {
            result = [fm removeItemAtURL:url error:&error];
        }
    }
}
