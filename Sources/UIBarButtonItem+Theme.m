...
+ (UIBarButtonItem *)themedDoneButtonWithTarget:(id)target andSelector:(SEL)selector
{
    ...
    if (SYSTEM_RUNS_IOS7_OR_LATER)
        ...
        [doneButton setBackgroundImage:[UIImage imageNamed:@"doneButtonHighlight"]
                            ...
                            barMetrics:UIBarMetricsDefault];
        [doneButton setTitleTextAttributes:@{NSShadowAttributeName : [UIColor whiteColor], NSForegroundColorAttributeName : [UIColor blackColor]}
                                  forState:UIControlStateNormal];
    }
    return doneButton;
}
...
+ (UIBarButtonItem *)themedBackButtonWithTarget:(id)target andSelector:(SEL)selector
{
    ...
    [backButton setTitleTextAttributes:@{NSShadowAttributeName : [UIColor colorWithWhite:0. alpha:.37], NSForegroundColorAttributeName : [UIColor whiteColor]} forState:UIControlStateNormal];
...
+ (UIBarButtonItem *)themedRevealMenuButtonWithTarget:(id)target andSelector:(SEL)selector
{
    ...
    UIBarButtonItem *menuButton = [[UIBarButtonItem alloc] initWithImage:icon style:/*UIBarButtonItemStyleBordered*/ UIBarButtonItemStylePlain target:target action:selector];
    if (SYSTEM_RUNS_IOS7_OR_LATER)
...
{
    UIBarButtonItem *button = [[UIBarButtonItem alloc] initWithTitle:title style:/*UIBarButtonItemStyleBordered*/ UIBarButtonItemStylePlain target:target action:selector];
    if (SYSTEM_RUNS_IOS7_OR_LATER)
